              ...
              <tr v-for="c in filteredCases" :key="c.id" class="case-row">
                ...
                <td class="dashboard-td">
                  <div class="flex items-center justify-center gap-8 cell-content">
                    ...
                    <NuxtLink :to="localePath({ path: c.case_type === 'refinement' ? '/refinement-submission' : '/case-submission', query: { id: c.id, mode: 'edit' } })" class="text-emerald-500 hover:text-emerald-700 transition-colors transform hover:scale-110" :title="$t('edit') || 'تعديل'">
                      <i class="fas fa-edit text-lg"></i>
                    </NuxtLink>
                    <button v-if="c.status !== 'incomplete submission' && c.case_type !== 'refinement'" @click.prevent="refineCase(c.id)" class="text-amber-500 hover:text-amber-700 transition-colors transform hover:scale-110" :title="$t('refinement')">
