<template>
  <div class="min-h-screen bg-gray-50 dark:bg-[#0f0f0f] pt-6 pb-6 transition-colors duration-300">
    <div class="container mx-auto px-4 max-w-7xl">
      ...
      <!-- Content -->
      <div v-else-if="caseData" class="space-y-4 animate-in fade-in slide-in-from-bottom-5 duration-700">
        
        <!-- Header / Quick Info -->
        <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl p-4 md:p-5 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
          <div class="absolute -top-10 -right-10 w-40 h-40 bg-brand-primary/5 rounded-full blur-3xl"></div>
          
          <div class="flex items-center gap-4 relative z-10">
            <div class="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 shadow-inner border border-slate-200 dark:border-slate-700 overflow-hidden">
              <i class="fas fa-hospital-user text-2xl mt-1"></i>
            </div>
            <div>
              <h1 class="text-xl md:text-2xl font-black text-slate-900 dark:text-white mb-1 uppercase">{{ caseData.case.patient_name }}</h1>
              <div class="flex flex-wrap items-center gap-2">
                <span class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-black tracking-widest uppercase border border-slate-200 dark:border-slate-700">
                  UUID: {{ caseData.case.uuid || ('#CASE-' + caseData.case.id) }}
                </span>
                <span :class="getStatusClass(caseData.case.status)" class="px-2 py-0.5 rounded-full text-white text-[10px] font-black tracking-widest uppercase">
          ...
          <div class="flex flex-wrap items-center gap-2 relative z-10">
            <button 
              ...
              class="flex-1 md:flex-none px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl transition-all shadow-sm flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 disabled:opacity-50"
            >
            ...
            <NuxtLink 
              ...
              class="flex-1 md:flex-none px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-black rounded-xl transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2"
            >
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
          <button 
            ...
            class="px-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all whitespace-nowrap border"
            :class="[
              activeTab === tab.id 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div :class="activeTab.startsWith('plan') ? 'lg:col-span-8' : 'lg:col-span-12'" class="space-y-4">
            <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl p-4 md:p-6 min-h-[200px]">
              
              <!-- Details Tab -->
              <div v-if="activeTab === 'details'" class="animate-in fade-in duration-500">
                <h3 class="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                  <i class="fas fa-info-circle text-brand-primary text-base"></i>
                  {{ $t('patient_case_informations') || 'Patient Case Informations' }}
                </h3>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  <div v-for="(val, label) in detailedInfo" :key="label" class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 transition-all hover:border-brand-primary/30">
                    <p class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-1">{{ label }}</p>
                    <p class="text-xs md:text-sm font-bold text-slate-900 dark:text-white">{{ val }}</p>
                  </div>
                </div>
              </div>
              ...
              <div v-if="activeTab === 'records'" class="animate-in fade-in duration-500">
                <h3 class="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <i class="fas fa-camera text-brand-primary text-base"></i>
                  {{ $t('patient_records') || 'Patient Records' }}
                </h3>
                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
                  <div v-for="(img, key) in recordImages" :key="key" class="group relative aspect-square rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                    ...
                    <div v-else class="w-full h-full flex flex-col items-center justify-center text-slate-300 dark:text-slate-600">
                      <i class="fas fa-image text-2xl mb-1.5"></i>
                      <span class="text-[9px] font-black uppercase tracking-tighter">{{ key }}</span>
                    </div>
                    <div v-if="img" class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                ...
                <!-- Chief Complaint & Additional Notes Section -->
                <div v-if="caseData.prescription?.chief_complaint || caseData.prescription?.additional_notes || caseData.case?.additional_instructions" class="mt-4 border-t border-slate-100 dark:border-slate-800 pt-4">
                  <div :class="[ (caseData.prescription?.chief_complaint && (caseData.prescription?.additional_notes || caseData.case?.additional_instructions)) ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-4' ]">
                    
                    <!-- Chief Complaint -->
                    <div v-if="caseData.prescription?.chief_complaint" class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 transition-all hover:border-brand-primary/30">
                      <h4 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                        <i class="fas fa-clipboard-list text-brand-primary text-xs"></i>
                        {{ $t('chief_complaint') || 'Chief Complaint' }}
                      </h4>
                      <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 whitespace-pre-line leading-relaxed">
                        {{ caseData.prescription.chief_complaint }}
                      </p>
                    </div>

                    <!-- Additional Notes -->
                    <div v-if="caseData.prescription?.additional_notes || caseData.case?.additional_instructions" class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 transition-all hover:border-brand-primary/30">
                      <h4 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                        <i class="fas fa-sticky-note text-brand-primary text-xs"></i>
                        {{ $t('additional_notes') || 'Additional Notes' }}
                      </h4>
                      <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 whitespace-pre-line leading-relaxed">
                        {{ caseData.prescription?.additional_notes || caseData.case?.additional_instructions }}
                      </p>
                    </div>
              ...
              <div v-if="activeTab === 'detailed_plan'" class="animate-in fade-in duration-500">
                <h3 class="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <i class="fas fa-list-check text-brand-primary text-base"></i>
                  {{ $t('detailed_treatment_plan') }}
                </h3>
                
                <div v-if="caseData.treatmentOptions?.length" class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden">
                  <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                      <thead>
                        <tr class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                          <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest w-12">#</th>
                          <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest w-40">{{ $t('plan_option') || 'Plan Option' }}</th>
                          <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('details') || 'Details' }}</th>
                          <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest w-28">{{ $t('date') || 'Date' }}</th>
                          <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest w-28">{{ $t('time') || 'Time' }}</th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
                        <tr v-for="(opt, idx) in filteredOptions" :key="idx" class="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors group">
                          <td class="px-4 py-2.5 text-xs font-bold text-slate-400">{{ idx + 1 }}</td>
                          <td class="px-4 py-2.5">
                            <span class="inline-flex items-center px-2 py-0.5 bg-brand-primary/10 text-brand-primary rounded-lg text-[9px] font-black uppercase tracking-wider group-hover:bg-brand-primary/20 transition-colors">
                              {{ opt.treatmnet_option_type }}
                            </span>
                          </td>
                          <td class="px-4 py-2.5">
                            <div class="space-y-2">
                              <p class="text-xs font-bold text-slate-700 dark:text-slate-300 leading-relaxed">{{ opt.treatmnet_option_details }}</p>
                              
                              <!-- Visual Teeth Display -->
                              <div v-if="getTeethForOption(opt.treatmnet_option_type).length" class="mt-2 p-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 max-w-lg">
              ...
              <div v-if="activeTab === 'history'" class="animate-in fade-in duration-500">
                <h3 class="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <i class="fas fa-history text-brand-primary text-base"></i>
                  {{ $t('case_history') || 'Case History' }}
                </h3>
                <div class="relative pl-6 border-l-2 border-slate-100 dark:border-slate-800 space-y-6 ml-3">
                  <div v-for="(item, idx) in caseData.history" :key="idx" class="relative">
                    <div class="absolute -left-[33px] top-1 w-4 h-4 rounded-full border-4 border-white dark:border-[#1a1a1a] transition-colors" :class="idx === 0 ? 'bg-brand-primary' : 'bg-slate-300 dark:bg-slate-700'"></div>
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-1 mb-1">
                      <span class="text-[10px] font-black text-brand-primary uppercase tracking-widest">{{ formatDate(item.created_at) }}</span>
                      <span class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] font-black uppercase">
                        Done by {{ (item.status || caseData.case.status || '').toLowerCase().includes('admin') || !['incomplete submission', 'complete submission', 'cancel', 'archived'].includes((item.status || caseData.case.status || '').toLowerCase()) ? 'Admin' : 'Doctor' }}
                      </span>
                    </div>
                    <h4 class="text-sm font-bold text-slate-900 dark:text-white uppercase">{{ item.status || caseData.case.status || 'complete submission' }}</h4>
                  </div>
                </div>
              </div>
              ...
              <div v-if="activeTab.startsWith('plan')" class="animate-in fade-in duration-500">
                <div class="bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 p-2 min-h-[400px] flex items-center justify-center relative group">
                  <iframe v-if="currentPlanLink" :src="currentPlanLink" class="w-full h-[480px] rounded-xl border-none shadow-2xl"></iframe>
                  <div v-else class="text-center">
                    <div class="w-16 h-16 bg-slate-200 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-3">
                      <i class="fas fa-cube text-2xl text-slate-400"></i>
                    </div>
                    <p class="text-slate-500 font-bold uppercase tracking-widest text-xs">3D Plan Viewer Not Available</p>
                  </div>
                </div>
              </div>
          ...
          <!-- Right Column: Patient Stats & Actions (Hidden when tab does not show plans) -->
          <div v-if="activeTab.startsWith('plan')" class="lg:col-span-4 space-y-4">
            
            <!-- Actions Card -->
            <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl p-4">
              <h3 class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-3 border-b border-slate-100 dark:border-slate-800 pb-2">Actions</h3>
              
              <div class="space-y-2">
                <button @click="handlePlanAction('approved')" :disabled="actionLoading" class="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-sm">
                  <i class="fas fa-check-circle"></i> Approve
                </button>
                <button @click="handlePlanAction('modify')" :disabled="actionLoading" class="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl shadow-lg shadow-amber-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-sm">
                  <i class="fas fa-edit"></i> Modify
                </button>
                <button @click="handlePlanAction('reject')" :disabled="actionLoading" class="w-full py-2.5 bg-red-500 hover:bg-red-600 text-white font-black rounded-xl shadow-lg shadow-red-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-sm">
                  <i class="fas fa-times-circle"></i> Reject
                </button>
              </div>

              <div v-if="currentPlanStatus" class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <p class="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-2">Current Status</p>
                <div :class="getTpClass(currentPlanStatus)" class="inline-block px-3 py-1 rounded-lg text-white text-[9px] font-black uppercase tracking-widest">
                  {{ currentPlanStatus }}
                </div>
              </div>
            </div>
...
const detailedInfo = computed(() => {
  ...
  const allInfo = {
    ...
    'Date of Birth': c.dob,
    'Gender': c.gender,
    'Treatment Plan 1': (c.treatment_plan1_status && c.treatment_plan1_status !== 'N/A' && c.treatment_plan1_status !== 'مرفق ملف') ? c.treatment_plan1_status : null,
    'Treatment Plan 2': (c.treatment_plan2_status && c.treatment_plan2_status !== 'N/A' && c.treatment_plan2_status !== 'مرفق ملف') ? c.treatment_plan2_status : null,
  }

  const filtered = {}
  for (const [key, val] of Object.entries(allInfo)) {
    if (val !== undefined && val !== null) {
      const strVal = String(val).trim()
      if (strVal !== '' && strVal !== '—' && strVal !== '-' && strVal.toLowerCase() !== 'not specified') {
        filtered[key] = val
      }
    }
  }
